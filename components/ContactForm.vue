...
<template>
  <form v-if="!submitted" class="grid gap-5" novalidate @submit.prevent="submit">
    ...
    <button class="site-form-button justify-self-start disabled:cursor-wait disabled:opacity-70" type="submit" :disabled="submitting" :aria-busy="submitting">{{ submitting ? 'Sending…' : 'Send Message' }} <span v-if="!submitting" aria-hidden="true">→</span></button>
  </form>
